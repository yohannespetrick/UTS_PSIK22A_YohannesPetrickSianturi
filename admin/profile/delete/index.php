      ...
      if (mysqli_num_rows($check_this_profile) == 0) {
        ...
        include("../../../lib/header.php");
          ...
          if (isset($_POST['delete'])) {
              $check_profile = mysqli_query($db, "SELECT profile_id FROM profile WHERE profile_id = '$post_id'");
              ...
              if (mysqli_num_rows($check_profile) == 0) {
                  ...
                  if(file_exists($target_delete_file)){ // cek apakah file yang ingin dihapus tersedia
                    ...
                    if($delete_old == TRUE){
                      $delete = mysqli_query($db, "DELETE FROM profile WHERE profile_id = '$post_id'");
                      if ($delete == TRUE) {
                          $msg_type = "success";
                          $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />- Profile berhasil dihapus.<script>swal("Berhasil!", "Profile berhasil dihapus.", "success");</script>';
                      } else {
                          $msg_type = "error";
...
<div class="row">
  ...
  <script>
  var url = "<?php echo $cfg_baseurl; ?>admin/profile/"; // URL Tujuan
        ...
        <div class="tab-pane fade show active" id="kelola-profile-tab">
            <form class="form-horizontal" role="form" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="category" autocomplete="off" class="form-control" value="<?php echo $data_this_profile['username']; ?>" readonly>
                </div>
                <div class="form-group">
                    <label class="control-label">Nama Lengkap</label>
                    <input type="text" name="profile_name" autocomplete="off" class="form-control" value="<?php echo $data_this_profile['profile_name']; ?>" readonly>
                </div>
                <div class="form-group">
