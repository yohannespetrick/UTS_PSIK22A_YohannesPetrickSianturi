...
if (isset($_SESSION['user'])) {
    ...
    if (mysqli_num_rows($check_user) == 0) {
        ...
        if (isset($_POST['add'])) {
            ...
            $check_about = mysqli_query($db, "SELECT title FROM about_us");
            if (empty($post_title) || empty($post_description)) {
                ...
                $msg_type = "error";
                $msg_content = '<b>Respon:</b> Gagal!<br /><b>Pesan:</b><br />- Data about sudah pernah diinput!.<script>swal("Gagal!", "Data about sudah pernah diinput!", "error");</script>';
                ...
                if(!empty($_FILES['image']['name'])){
                    ...
                    if($upload == TRUE){
                        $insert = mysqli_query($db, "INSERT INTO about_us (title, description, image, update_at, update_by) VALUES ('$post_title', '$post_description', '$set_new_filename', '$date', '$user_id')");
                        if ($insert == TRUE) {
                            $msg_type = "success";
                            $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />About berhasil disimpan.<script>swal("Berhasil!", "About Us berhasil disimpan.", "success");</script>';
                        } else {
                            $msg_type = "error";
        ...
        <div class="tab-pane fade show active" id="kelola-about-tab">
            <form class="form-horizontal" role="form" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" autocomplete="off" class="form-control" placeholder="Masukkan judul.." required>
                    <p id="limit_title"></p>
                </div>
                <div class="form-group">
                    <label class="control-label">Deskripsi</label>
                    <textarea name="description" id="description" rows="10" autocomplete="off" class="form-control" placeholder="Masukkan deskripsi.." required></textarea>
