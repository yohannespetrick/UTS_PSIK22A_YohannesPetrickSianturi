      ...
      if (mysqli_num_rows($check_this_about) == 0) {
        ...
        include("../../../lib/header.php");
          ...
          if (isset($_POST['delete'])) {
              $check_about = mysqli_query($db, "SELECT about_id FROM about_us WHERE about_id = '$post_id'");
              $cek_old_file = $data_this_about['image'];
              if (mysqli_num_rows($check_about) == 0) {
                  $msg_type = "error";
                  ...
                  if(file_exists($target_delete_file)){ // cek apakah file yang ingin dihapus tersedia
                    ...
                    if($delete_old == TRUE){
                      $delete = mysqli_query($db, "DELETE FROM about_us WHERE about_id = '$post_id'");
                      if ($delete == TRUE) {
                          $msg_type = "success";
                          $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />- About berhasil dihapus.<script>swal("Berhasil!", "About berhasil dihapus.", "success");</script>';
                      } else {
                          $msg_type = "error";
...
<div class="row">
  ...
  <script>
  var url = "<?php echo $cfg_baseurl; ?>admin/about/"; // URL Tujuan
        ...
        <div class="tab-pane fade show active" id="kelola-about-tab">
            <form class="form-horizontal" role="form" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="title" autocomplete="off" class="form-control" value="<?php echo $data_this_about['title']; ?>" readonly>
                </div>
                <div class="form-group">
                    <label class="control-label">Gambar</label>
                    <img src="<?php echo $cfg_baseurl; ?>assets/images/about/<?php echo $data_this_about['image']; ?>" class="form-control" width="100%" height="250">
                </div>
                <div class="form-group">
