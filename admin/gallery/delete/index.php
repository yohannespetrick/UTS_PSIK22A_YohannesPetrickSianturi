        ...
        if (isset($_GET['id'])) {
            ...
            if (mysqli_num_rows($check_this_gallery) == 0) {
                ...
                if (isset($_POST['delete'])) {
                    $check_gallery = mysqli_query($db, "SELECT gallery_id FROM gallery WHERE gallery_id = '$post_id'");
			        ...
			        if (mysqli_num_rows($check_gallery) == 0) {
			            ...
			            if(file_exists($target_delete_file)){
			                ...
			                if($delete_old == TRUE){
			                    $delete = mysqli_query($db, "DELETE FROM gallery WHERE gallery_id = '$post_id'");
			                    if ($delete == TRUE) {
			                        $msg_type = "success";
			                        $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />- Galeri berhasil dihapus.<script>swal("Berhasil!", "Galeri berhasil dihapus.", "success");</script>';
			                    } else {
			                        $msg_type = "error";
...
<div class="row">
  ...
  <script>
  var url = "<?php echo $cfg_baseurl; ?>admin/gallery/"; // URL Tujuan
        ...
        <div class="tab-pane fade show active" id="kelola-gallery-tab">
            <form class="form-horizontal" role="form" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <img src="<?php echo $cfg_baseurl; ?>assets/images/gallery/<?php echo $data_this_gallery['image']; ?>" class="form-control" width="100%" height="250">
                </div>
                <div class="form-group">
