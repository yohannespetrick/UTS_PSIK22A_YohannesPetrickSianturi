		...
		if (isset($_GET['id'])) {
			...
			if (mysqli_num_rows($check_this_testimonial) == 0) {
				...
				if (isset($_POST['edit'])) {
                    ...
                    $check_customer_name = mysqli_query($db, "SELECT customer_name FROM testimonial WHERE customer_name = '$post_customer_name' AND testimonial_id != '$post_id'");
                    ...
                    } else if (mysqli_num_rows($check_customer_name) > 0) {
                        $msg_type = "error";
                        $msg_content = '<b>Respon:</b> Gagal!<br /><b>Pesan:</b><br />- Data '.$post_customer_name.' sudah pernah terdaftar.<script>swal("Gagal!", "Data '.$post_customer_name.' sudah pernah terdaftar!", "error");</script>';
                        ...
                        if(!empty($_FILES['image']['name'])){
                            ...
                            if(file_exists($target_delete_file)){ // cek apakah file yang ingin dihapus tersedia
                                ...
                                if($delete_old == TRUE){
                                  ...
                                  if($upload == TRUE){
                                      $update = mysqli_query($db, "UPDATE testimonial SET customer_name = '$post_customer_name', description = '$post_description', image = '$set_new_filename', update_at = '$date', update_by = '$user_id' WHERE testimonial_id = '$post_id'");
                                      if ($update == TRUE) {
                                        $msg_type = "success";
                                        $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />Testimonial berhasil diperbarui.<script>swal("Berhasil!", "Testimonial berhasil diperbarui.", "success");</script>';
                                      } else {
                                        $msg_type = "error";
                            ...
                            $update = mysqli_query($db, "UPDATE testimonial SET customer_name = '$post_customer_name', description = '$post_description', update_at = '$date', update_by = '$user_id' WHERE testimonial_id = '$post_id'");
                            if ($update == TRUE) {
                                $msg_type = "success";
                                $msg_content = '<b>Respon:</b> Berhasil!<br /><b>Pesan:</b><br />Testimonial berhasil diperbarui.<script>swal("Berhasil!", "Testimonial berhasil diperbarui.", "success");</script>';
                            } else {
...
<div class="row">
  ...
  <script>
  var url = "<?php echo $cfg_baseurl; ?>admin/testimonial/edit/?id=<?php echo $post_id; ?>"; // URL Tujuan
        ...
        <div class="tab-pane fade show active" id="kelola-testimonial-tab">
            <form class="form-horizontal" role="form" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="customer_name" autocomplete="off" class="form-control" value="<?php echo $data_this_testimonial['customer_name']; ?>" required>
                </div>
                <div class="form-group">
                    <label class="control-label">Deskripsi</label>
                    <textarea name="description" id="description" rows="6" autocomplete="off" class="form-control" required><?php echo $data_this_testimonial['description']; ?></textarea>
                </div>
                <div class="form-group">
                    <small>Gambar saat ini : <a href="<?php echo $cfg_baseurl; ?>assets/images/testimonial/<?php echo $data_this_testimonial['image']; ?>" class="text-primary"><?php echo $data_this_testimonial['image']; ?></a></small>
                </div>
                <div class="form-group">
                  <label class="control-label">Ganti Foto Pelanggan</label>
                  <input type="file" name="image" autocomplete="off" class="form-control" placeholder="Masukkan gambar..">
                  <p class="help-block">
