...
if($_GET['p']){
    ...
    $query_news_filter = "SELECT title,image,publish_at,category_id,description,fullname FROM news,users WHERE news.update_by = users.user_id AND tautan = '$data' LIMIT 1"; // edit
...
?>
                        <div class="row">
                          <center>
                          <div class="col-md-10" style="margin-top: 10px; margin-bottom: 5px;">
                          ...
                          <?php
                          $query_news_related = "SELECT title,image,publish_at,category_id,description,tautan FROM news ORDER BY rand() LIMIT 6"; // edit
...
?>
                                ...
                                <div id="carouselfaded" class="carousel slide carousel-fade carousel-light" data-bs-ride="carousel" data-bs-theme="light">
                                  <div class="carousel-inner text-center">
                                    ...
                                    <div class="carousel-item">
                                      <img src="<?php echo $cfg_baseurl; ?>assets/images/banner/<?php echo $data_banner['image']; ?>" alt="<?php echo $data_banner['title']; ?>" class="content-rounded-type" width="100%" height="250">
                                    </div>
                                    <?php 
                                        }
                                    }
                                ...
                                <div class="container">
                                  <div class="testimonial mySwiper">
                                    <div class="testi-content swiper-wrapper">
                                      <?php
                                      $query_testi = mysqli_query($db, "SELECT * FROM testimonial ORDER BY testimonial_id DESC");
                                      if(mysqli_num_rows($query_testi) > 0){
                                          while($data_testi = mysqli_fetch_assoc($query_testi)){
                                      ?>
                                      <div class="slide swiper-slide">
                                        <img src="<?php echo $cfg_baseurl; ?>assets/images/testimonial/<?php echo $data_testi['image']; ?>" class="image" />
                                        <p>
                                          <?php echo nl2br($data_testi['description']); ?>.
                                        </p>
                            
                                        <div class="text-primary">
                                          <span class="name"><?php echo $data_testi['customer_name']; ?></span>
                                        </div>
                                      </div>
                                      <?php
                                          }
                        ...
                        <div class="row">
                          ...
                          <?php
                          $query_news_latest = "SELECT title,image,publish_at,category_id,description,tautan FROM news ORDER BY rand() LIMIT 3"; // edit
                          $exec_news_latest = mysqli_query($db, $query_news_latest);
                          if(mysqli_num_rows($exec_news_latest) > 0){
                            while($data_news_latest = mysqli_fetch_assoc($exec_news_latest)){
                              $content = $data_news_latest['description'];
                              $desc = substr($content, 0, 100);
                          ?>
                        	<div class="col-md-4" style="margin-top: 10px;">
                        		<div class="wow animate__animated animate__fadeIn">
                        			<a class="text-decoration-none wow animate__bounceIn" href="<?php echo $cfg_baseurl; ?>article/?p=<?php echo $data_news_latest['tautan']; ?>" style="visibility: visible;">
                        				<div class="card artikel-t">
                        					<img class="card-img-top card-img-bottom img-fluid" src="<?php echo $cfg_baseurl; ?>assets/images/news/<?php echo $data_news_latest['image']; ?>">
                        					<div class="card-body">
                        						<h4 class="card-title text-decoration-none"><font color="black"><?php echo $data_news_latest['title']; ?></font></h4>
                                    <p class="card-text text-dark small"><?php echo $desc; ?>.... <span class="text-primary">(Baca Selengkapnya)</span></p>
                                    <p class="card-text small text-right text-primary"><i><?php echo format_date($data_news_latest['publish_at']); ?></i></p>
                        					</div>
                        				</div>
                        			</a>
                        		</div>
